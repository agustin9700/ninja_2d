...
function buildCommands() {
  ...
  const facing = timeline === state.runTimeline && runAnim.active ? runAnim.dir : 1;
...
function playbackBounds() {
  ...
  for (const timeline of [state.idleTimeline, state.crouchTimeline, state.runTimeline, state.jumpTimeline, state.attackTimeline, state.hitTimeline, state.deathTimeline].filter(Boolean)) {
    const directions = timeline === state.runTimeline ? [1, -1] : [1];
...
function drawPart(command) {
  const image = state.images.get(command.partName);
  ...
  ctx.globalAlpha = Math.max(0, Math.min(1, finite(command.element.colorAlphaPercent, 100) / 100));
...
async function loadManifest() {
  try {
    ...
    state.loaded = false;
    setStatus(`${error.message}\n\nCopiá asset_manifest.json y los PNG dentro de prototype/assets/.`);
  }
}
